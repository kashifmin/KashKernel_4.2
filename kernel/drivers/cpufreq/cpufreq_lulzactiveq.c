...
static void cpufreq_lulzactive_timer(unsigned long data)
{
	...
	if (dbs_tuners_ins.ignore_nice) {
		...
		cur_nice = cputime64_sub(kcpustat_cpu(data).cpustat[CPUTIME_NICE],
...
static int cpufreq_lulzactive_up_task(void *data)
{
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (dbs_tuners_ins.dvfs_debug) {
				printk (KERN_ERR "LulzQ: [FREQ UP] CPU %d, Target Up Frequency: %u\n",
								 cpu, max_freq);
			}
...
static void cpufreq_lulzactive_freq_down(struct work_struct *work)
{
	...
	for_each_cpu(cpu, &tmp_mask) {
		...
		if (dbs_tuners_ins.dvfs_debug) {
			printk (KERN_ERR "LulzQ: [FREQ DOWN] CPU %d, Target Down Frequency: %u\n",
							 cpu, max_freq);
		}
