	...
	if(1 == sscanf(buf, "%d", &no_early_suspend))
	{
        ...
        if (1 == sensor_suspend)
        {
            ...
            if (FALSE == s_g_stop_early_suspend
                && 1 == no_early_suspend)
            {
                GSE_ERR("Jiangde BMA222_power(obj->hw, 1); \n");
                ...
                do {
                	...
                	{
                		GSE_ERR("Jiangde before BMA222_SetPowerMode databuf = 0x%x\n",databuf[0]);
                	}

                    GSE_ERR("Jiangde bma222_init_client(obj->client, 0) \n");
                    if(bma222_init_client(obj->client, 0))
                    {
                        GSE_ERR("Jiangde initialize client fail!!\n");
                        break;        
                    }
                    
                    GSE_ERR("Jiangde BMA222_SetPowerMode(obj->client, true) \n");
                	if(BMA222_SetPowerMode(obj->client, true))
                	{
                	...
                	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
                	{
                		GSE_ERR("Jiangde after BMA222_SetPowerMode databuf = 0x%x\n",databuf[0]);
                	}
                } while(0);
                mutex_unlock(&bma222_mutex); 
            }
                ...
                && 0 == no_early_suspend)
            {
                ...
                do  {
                	...
                	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
                	{
                        GSE_ERR("Jiangde before BMA222_SetPowerMode in suspend databuf = 0x%x\n",databuf[0]);
                	}
                    
                    GSE_ERR("Jiangde BMA222_SetPowerMode(obj->client, false) \n");
                	if(BMA222_SetPowerMode(obj->client, false))
                	{
                		GSE_ERR("Jiangde write power control fail!!\n");
                		break;
                	}
                	...
                	if(databuf[0]==0xff)//if the value is ff the gsensor will not work anymore, any i2c operations won't be vaild
                	{
                        GSE_ERR("Jiangde after BMA222_SetPowerMode suspend err databuf = 0x%x\n",databuf[0]);
                	}
                    
                    GSE_ERR("Jiangde BMA222_power(obj->hw, 0); \n");
...
static void bma222_late_resume(struct early_suspend *h)
{
	...
	int err;
  	...
  	GSE_ERR("JiangdetestT Gsensor bma222_late_resume \n");
	if(obj == NULL)
	{
		GSE_ERR("null pointer!!\n");
		return;
