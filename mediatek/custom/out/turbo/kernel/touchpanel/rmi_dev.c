...
//LINE<JIRA_ID><DATE20130608><BUG_INFO>zenghaihui
// For Firmware Update 
#define SYNAPTICS_IOCTLID	0xE0
#define SYNAPTICS_IOCTL_FW_UPDATE  _IOR(SYNAPTICS_IOCTLID, 1, int)
#define SYNAPTICS_IOCTL_TP_UPGRADE_SET_BIN_BUF  _IOR(SYNAPTICS_IOCTLID, 2, int)
#define SYNAPTICS_IOCTL_TP_UPGRADE_SET_BIN_LEN  _IOR(SYNAPTICS_IOCTLID, 3, int)
...
static struct device_attribute attrs[] = {
	__ATTR(open, S_IRUGO,//S_IWUGO, //change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			rmidev_sysfs_open_store),
	__ATTR(release, S_IRUGO,//S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			rmidev_sysfs_release_store),
	__ATTR(address, S_IRUGO,//S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			rmidev_sysfs_address_store),
	__ATTR(length, S_IRUGO,//S_IWUGO,//change by panchenjun for cts test failed 20130722
			synaptics_rmi4_show_error,
			rmidev_sysfs_length_store),
	__ATTR(data, (S_IRUGO /* | S_IWUGO change by panchenjun for cts test failed 20130722*/),
			rmidev_sysfs_data_show,
			rmidev_sysfs_data_store),
	__ATTR(attn, (S_IRUGO /* | S_IWUGO change by panchenjun for cts test failed 20130722*/),
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d %d\n", CUST_EINT_TOUCH_PANEL_SENSITIVE, attn);
}
...
//LINE<JIRA_ID><DATE20130608><BUG_INFO>zenghaihui
static uint8_t *synaptics_file_fw_data = NULL;
static int synaptics_file_fw_data_len = 0;
...
int synaptics_fw_upgrade_from_file(void)
{
    int ret = -1;
    
    printk("[synaptics]  Entry synaptics_fw_upgrade_from_file ");
    
    disable_irq(CUST_EINT_TOUCH_PANEL_NUM);
    mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
    
    ret = synaptics_fw_updater(synaptics_file_fw_data);
    ...
    if(ret >= 0)
    {
        return synaptics_file_fw_data_len;
    }
...
static int synaptics_isp_ioctl(/*struct inode *inode,*/ struct file *filp,    unsigned int cmd, unsigned long arg)
{
    int __user *ip = (int __user *)arg;
    CTP_DBG("[synaptics]into synaptics_isp_ioctl\n");
    CTP_DBG("cmd value %x\n",cmd);
    
    switch (cmd) {        
        case SYNAPTICS_IOCTL_FW_UPDATE:
            
                printk("[synaptics] tp upgrade: synaptics_file_fw_data=%x, synaptics_file_fw_data_len = %x", 
                synaptics_file_fw_data, synaptics_file_fw_data_len);
                
                if((synaptics_file_fw_data == NULL) || (synaptics_file_fw_data_len <= 0))
                {
                    return 0;
                }
                else
                {
                    int ret = 0;
                    ret = synaptics_fw_upgrade_from_file();
                    printk("[synaptics]  synaptics_fw_upgrade_from_file ret =%x ", ret);

                    synaptics_file_fw_data = NULL;
                    synaptics_file_fw_data_len = 0;
            ...
            case SYNAPTICS_IOCTL_TP_UPGRADE_SET_BIN_BUF:
                synaptics_file_fw_data = (uint8_t *)arg;
                break;
            
            case SYNAPTICS_IOCTL_TP_UPGRADE_SET_BIN_LEN:
                synaptics_file_fw_data_len = (int)arg;
                break;
...
static const struct file_operations rmidev_fops = {
	...
	.release = rmidev_release,
        .unlocked_ioctl=synaptics_isp_ioctl, 
};
