...
#define SENSORDB(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV5648MIPI(sunny)]", fmt, ##arg)
#else
...
static UINT32 OV5647MIPISetMaxFrameRate(UINT16 u2FrameRate);
...
static kal_uint16 OV5647MIPI_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
static kal_uint16 OV5647MIPI_read_cmos_sensor(kal_uint32 addr)
{
...
static kal_uint16 OV5647MIPI_check_otp_wb(kal_uint16 index)
{
...
//index:index of otp group.(0,1,2)
//return: 0
static kal_uint16 OV5647MIPI_read_otp_wb(kal_uint16 index, struct OV5647MIPI_otp_struct *otp)
{
	kal_uint16 temp;
	kal_uint32 address;
       OV5647MIPI_write_cmos_sensor(0x0100, 0x01);	
...
//reutrn 0
static kal_uint16 OV5647MIPI_update_wb_gain(kal_uint32 R_gain, kal_uint32 G_gain, kal_uint32 B_gain)
{
...
static kal_uint16 OV5647MIPI_update_wb_register_from_otp(void)
{
...
static kal_uint16 OV5647MIPI_check_otp_lenc(kal_uint16 index)
{
   kal_uint16 temp,flag;
   kal_uint32 address;
...
static kal_uint16 OV5647MIPI_read_otp_lenc(kal_uint16 index,struct OV5647MIPI_otp_struct *otp)
{
...
//return 0
static kal_uint16 OV5647MIPI_update_lenc(struct OV5647MIPI_otp_struct *otp)
{
	kal_uint16 i, temp;
	//lenc g
	for(i = 0; i < 36; i++)
...
static kal_uint16 OV5647MIPI_update_lenc_register_from_otp(void)
{
	kal_uint16 temp,i,otp_index;
    struct OV5647MIPI_otp_struct current_otp;
	...
	spin_unlock_irqrestore(&ov5647mipi_drv_lock,flags);
		if((CurrentScenarioId == MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG)||(CurrentScenarioId == MSDK_SCENARIO_ID_CAMERA_ZSD)){
		  dummy_line = FrameHeight - OV5647MIPI_FULL_PERIOD_LINE_NUMS;
			...
			#ifdef OV5647_USE_VIDEO_ZOOM //20130517 modified by mingji
...
*
*************************************************************************/
static void set_OV5647MIPI_shutter(kal_uint16 iShutter)
{
	unsigned long flags;
 ...
 */
 ...
 static kal_uint8 OV5647MIPIGain2Reg(const kal_uint16 iGain)
{
...
*
*************************************************************************/
static kal_uint16 OV5647MIPI_SetGain(kal_uint16 iGain)
{
	kal_uint8 iReg;
...
*
*************************************************************************/
static void OV5647MIPI_night_mode(kal_bool enable)
{
...
static UINT32 OV5647MIPIOpen(void)
{
...
*
*************************************************************************/
static kal_uint16 OV5647MIPICheckSensorMID(void)
{
	kal_uint16 temp, i, otp_index;
	//struct OV5647MIPI_otp_struct current_otp;
	//kal_uint32 R_gain, B_gain, G_gain, G_gain_R,G_gain_B;
	...
	SENSORDB("OV5647MIPICheckSensorMID\n");
	...
	address = 0x3d05 +otp_index*9;

	////4 modified the start address
	//address = 0x05 + index*9 +1;

	customer_id = (OV5647MIPI_read_cmos_sensor(address)&0x7f);
	SENSORDB("[OV5647MIPI_read_otp_wb]address[%x]module_integrator_id[%x]\n",address,customer_id);
...
static UINT32 OV5647MIPIGetSensorID(UINT32 *sensorID) 
{
		// check if sensor ID correct
	*sensorID=((OV5647MIPI_read_cmos_sensor(0x300A) << 8) | OV5647MIPI_read_cmos_sensor(0x300B));	
    ...
    {
            if (1 == OV5647MIPICheckSensorMID())
            {
                printk("This is sunny ov5647 module.\n");
                *sensorID = OV5648MIPI_SENSOR_ID;
                return ERROR_NONE;
            }
            else
            {
                printk("This is other OV5647 module.\n");
		        *sensorID=0xFFFFFFFF;//line<20130402>wangyanhui add 
		        return ERROR_SENSOR_CONNECT_FAIL;
            }
    }
    else
...
*
*************************************************************************/
static UINT32 OV5647MIPIClose(void)
{
#ifdef OV5647MIPI_DRIVER_TRACE
...
*
*************************************************************************/
static UINT32 OV5647MIPIPreview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
...
static UINT32 OV5647MIPIZsdPreview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...
static UINT32 OV5647MIPIVIDEO(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	kal_uint16 dummy_line=0;
...
*
*************************************************************************/
static UINT32 OV5647MIPICapture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...
static UINT32 OV5647MIPI3DPreview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...
static UINT32 OV5647MIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	#else
	pSensorResolution->SensorVideoWidth=OV5647MIPI_IMAGE_SENSOR_PV_WIDTH;
	pSensorResolution->SensorVideoHeight=OV5647MIPI_IMAGE_SENSOR_PV_HEIGHT;
	#endif
...
static UINT32 OV5647MIPIGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,
  ...
  switch(ScenarioId)
    {
		...
		#ifndef OV5647_USE_VIDEO_ZOOM //20130517 modified by mingji
...
static UINT32 OV5647MIPIControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
...
static UINT32 OV5647MIPISetVideoMode(UINT16 u2FrameRate)
{
...
static UINT32 OV5647MIPISetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
...
static UINT32 OV5647MIPISetCalData(PSET_SENSOR_CALIBRATION_DATA_STRUCT pSetSensorCalData){
	UINT32 i;
...
static UINT32 OV5647MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	kal_uint32 pclk;
	kal_int16 dummyLine;
...
static UINT32 OV5647MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) {
...
static UINT32 OV5647MIPIFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId,
...
static SENSOR_FUNCTION_STRUCT	SensorFuncOV5647MIPI=
{
	OV5647MIPIOpen,
	OV5647MIPIGetInfo,
	OV5647MIPIGetResolution,
