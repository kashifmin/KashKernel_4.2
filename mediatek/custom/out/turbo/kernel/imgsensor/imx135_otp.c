...
#undef CDBG
#define CDBG(fmt, args...) printk(KERN_INFO "imx135_otp.c: " fmt, ## args)
...
static uint32_t R_gain1=0x100,B_gain1=0x100,G_gain1=0x100;
...
static uint32_t RG_Ratio_typical_Sunny=0x0282;
static uint32_t BG_Ratio_typical_Sunny=0x0231;
static uint32_t GG_Ratio_typical_Sunny=0x0402;
...
int update_otp_wb(void)
{
    ...
    if(0 == wb_flag)
    {
...
#ifdef IMX135_OTP_DEBUG
        printk("HJDDBG, [mingji] R_gain == 0x%x\n",R_gain);
        printk("HJDDBG, [mingji] G_gain == 0x%x\n",G_gain);
        printk("HJDDBG, [mingji] B_gain == 0x%x\n",B_gain);
#endif
        if (R_gain < 0x100)
        {
            R_gain = 0x100;
        ...
        #ifdef IMX135_OTP_DEBUG
        printk("HJDDBG, [mingji] R_gain1 == 0x%x\n",R_gain1);
        printk("HJDDBG, [mingji] G_gain1 == 0x%x\n",G_gain1);
        printk("HJDDBG, [mingji] B_gain1 == 0x%x\n",B_gain1);

        printk("HJDDBG, [mingji] R_gain1&0xFF == 0x%x\n",R_gain1&0xFF);
        printk("HJDDBG, [mingji] G_gain1&0xFF == 0x%x\n",G_gain1&0xFF);
        printk("HJDDBG, [mingji] B_gain1&0xFF == 0x%x\n",B_gain1&0xFF);
        #endif
    }
...
// BEGIN <><20130710><sunny otp> Jiangde 
bool is_sunny_p13n03h(void)
{
    uint i, temp, otp_index;
    struct otp_struct current_otp = {0};
	
     // check first wb OTP with valid data
    printk("[Sunny] in is_sunny_p13n03h\n");
    for(i=2; i>0; i--) {
        temp = check_otp_awb(i);
        if (temp == 2) {
            otp_index = i;
    ...
    if(i==0) {
        // no valid wb OTP data
        printk("[Sunny] no valid wb OTP data, error !!!\n");
        return 0;
    }
    ...
    {
        //add for distinguish diffrent moudle Truly or Liteon
        printk("[Sunny] ######### current_otp->customer_id = 0x%x(ModuleID)\n", current_otp.customer_id);

        if(current_otp.customer_id == 0x01)
        {   
            printk("[Sunny] out is_sunny_p13n03h, OK\n");            
            return 1;
        }
    }
   
    printk("[Sunny] out is_sunny_p13n03h, error !!!\n");            
    return 0;
}
// END <><20130710><sunny otp> Jiangde         
