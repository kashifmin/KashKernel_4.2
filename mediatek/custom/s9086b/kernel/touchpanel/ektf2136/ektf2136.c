...
static int ctp_suspend= -1; // show that: ctp is power_down or not 1:suspend  -1:resume
...
static void elan_esd_check(struct work_struct *work)
{
	...
	if(work_lock ||elan_status) //tp is upgrading...
	{
		 TPD_DEBUG("elan_esd_check -- CTP is upgrading , work_lock == %d, elan_status == %d ,exit elan_esd_check....\n",work_lock,elan_status);
	...
	if (rc != 4)
	{
		...
		if(buf[0] != 0x52){
			...
			printk("elan esd respone normal\n");
			mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
			printk("elan esd ctp_suspend == %d\n",ctp_suspend);
			//if(!ctp_suspend)
		  		queue_delayed_work(elan_esd_check_workqueue, &elan_esd_check_work, TPD_ESD_CHECK_CIRCLE);
			mutex_unlock(&elan_mutex);	
			return;
