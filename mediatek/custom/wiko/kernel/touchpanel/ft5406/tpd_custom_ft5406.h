...
#define TINNO_TOUCH_TRACK_IDS 10    // 5

typedef struct _tinno_ts_point{
	int x, y, pressure, flag, touch_id, status;
}tinno_ts_point;


#define FTS_PROTOCOL_LEN 61 // 61 for 10 points,   31 for 5 points
...
typedef struct {
	uint8_t start_reg;
	uint8_t buffer[FTS_PROTOCOL_LEN + 1];
...
#define FTS_CTP_FIRWARE_ID (0x7903) //0x7903---FT5X06, 0x7907---FT5X16, 0x7903---FT5406
...
#define CTP_DBG(fmt, arg...) \
	printk("[CTP-FT5406] %s (line:%d) :" fmt "\r\n", __func__, __LINE__, ## arg)
#else
