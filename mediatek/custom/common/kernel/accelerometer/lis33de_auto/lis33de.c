...
static int lis33de_local_init(void);
static int lis33de_remove(void);

static int lis33de_init_flag =0; // 0<==>OK -1 <==> fail
...
static struct sensor_init_info lis33de_init_info = {
		.name = "lis33de",
		.init = lis33de_local_init,
		.uninit = lis33de_remove,
...
static int lis33de_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = lis33de_get_cust_acc_hw();
	...
	if((err = lis33de_create_attr(&(lis33de_init_info.platform_diver_addr->driver))))
	{
...
static int lis33de_i2c_remove(struct i2c_client *client)
{
	...
	if(err = lis33de_delete_attr(&(lis33de_init_info.platform_diver_addr->driver)))
	{
...
{
	struct acc_hw *hw = lis33de_get_cust_acc_hw();
	GSE_FUN();
...
{
	GSE_FUN();
	hwmsen_gsensor_add(&lis33de_init_info);
