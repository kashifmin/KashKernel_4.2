...
*/
	...
	res = LIS3DH_SetBWRate(client, LIS3DH_BW_100HZ|0x8F);//400 or 100 no other choice
	if(res != LIS3DH_SUCCESS )
	{
		return res;
	}
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			else
			{
				...
				if(value <= 5)
				{
					sample_delay = LIS3DH_BW_200HZ|0x8F;
				}
				else if(value <= 10)
				{
					sample_delay = LIS3DH_BW_100HZ|0x8F;
				}
				else
				{
					sample_delay = LIS3DH_BW_50HZ|0x8F;
				}
...
static void lis3dh_early_suspend(struct early_suspend *h) 
{
	...
	*/
	...
	if(hwmsen_read_byte(obj->client, addr, &databuf[0]))
	{
		GSE_ERR("read power ctl register1 err!\n");
		return;
	}
	databuf[1] = databuf[0]&0x7F;
	databuf[0] = LIS3DH_REG_CTL_REG1;
	...
	GSE_ERR("power control REG1:0x%x!!\n",databuf[0]);
	LIS3DH_power(obj->hw, 0);
}
/*----------------------------------------------------------------------------*/
